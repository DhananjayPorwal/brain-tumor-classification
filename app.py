...
def main():
    ...
    st.header(
        "Making computers learn to predict, if an image contains traces of tumor.")
    ...
    if option_1 or option_2:
        uploaded_file = st.file_uploader(
            "Choose a brain MRI image for classification as tumor or non-tumor...", type=image_types)
        if uploaded_file is not None:
            ...
            if option_1:
                st.write(
                    "You have chosen VGG model for prediction, classifying now. ")
                st.spinner()
                ...
                if label == 1:
                    st.write(
                        "Result: Apparently, this scan has a tumor. Please consult a doctor for prescription.")
                else:
                    st.write(
                        "Result: Seemingly, it shows no traces of a tumor presence. Take good care.")
            if option_2:
                st.write("")
                st.write("")
                st.write("")
                st.write(
                    'You have selected ResNet model for prediction, classifying now.')
                st.spinner()
                ...
                if label == 1:
                    st.write(
                        "Result: Seemingly, this scan caught a tumor. Please visit a doctor for prescription.")
                else:
                    st.write(
                        "Result: Apparently, no tumor exists yet. Take good care.")
